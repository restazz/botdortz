...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `salvee @${num.split('@')[0]}\ seja bem vindo(a) ao grupo *${mdata.subject}* comportasse para nao ser banido`
				...
				teks = `@${num.split('@')[0]} saiu porque e broxa`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❗❭ AGUARDE QUE ESTÁ EM PRODUÇÃO (CASO DEMORAR DEU ERRO) ❬❗❭',
				success: '️❬ ✔ ❭ PRONTINHO 🖤',
				error: {
				...
				only: {
					group: '❬❗❭ Apenas os adms do grupo pode usar este comando ',
					ownerG: '❬❗❭ Apenas os adms do grupo pode usar este comando ',
					ownerB: '❬❗❭  Apenas os adms do grupo pode usar este comando ',
					admin: '❬❗❭ Apenas os adms do grupo pode usar este comando ',
					Badmin: '❬❗❭O bot precisa ser Admin ;3 '
				}
			}
            ...
            case 'kangcopet':
				...
				case 's':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(` [❗] vídeo muito longo (limite máximo de 9 segundos) [❗] }
					break
				case 'getses':
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Não respeitou e perdeu o adm :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(` Novo adm na área, bora respeitar @${mentioned[0].split('@')[0]} `, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Novo adm na área, bora respeitar :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Não seguiu as regras é foi de base :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`@${mentioned[0].split('@')[0]} Não seguiu as regras é foi de base`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('[❗] algo deu errado, tente novamente [❗]')
